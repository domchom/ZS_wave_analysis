...
class TotalSignalProcessor:
    ...
    def calc_peak_props(self):
        ...
        and polynomial order 2. The peak properties are calculated by finding peaks 
            ...
            with tqdm(total=its, miniters=its/100) as pbar:
                ...
                for combo_number, combo in enumerate(self.channel_combos):
                    for col in range(self.num_cols):
                        pbar.update(1)
                        ch1_normalized = normalize(self.indv_line_values[combo[0], col, :])
                        ch2_normalized = normalize(self.indv_line_values[combo[1], col, :])
                        ccf_curve = self.indv_ccfs[combo_number, col]

                        # Saving measurements to a CSV file for each box
                        measurements = list(zip_longest(range(1, len(ccf_curve) + 1), ch1_normalized, ch2_normalized, ccf_curve, fillvalue=None))
                        filename = f'Box{col + 1}_CCF_values.csv'
                        save_indv_CCFs_to_csv(measurements, filename)
