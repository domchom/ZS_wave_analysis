...
class TotalSignalProcessor:
    ...
    def plot_mean_latent_periods(self):
        def return_figure(latent_per_arr: np.ndarray, Ch_name: str):
            # Filter nans out of the array
            latent_per_arr = [val for val in latent_per_arr if not np.isnan(val)]

            fig, ax = plt.subplots()

            # Plot histogram
            ax.hist(latent_per_arr, color='tab:blue', label='latent_period', alpha=0.75)
            ax.set_xlabel(f'{Ch_name} latent period values')
            ax.set_ylabel('Occurances')
            ax.set_title('Histogram of Latent Period')

            plt.close(fig)
            return fig

        # empty dictionary to fill with figures, in the event that we make more than one
        self.latent_period_figs = {}

        # fill the dictionary with plots for each channel
        if hasattr(self, 'acfs'):
            for channel in range(self.num_channels):
                self.latent_period_figs[f'Ch{channel + 1} Latent Period'] = return_figure(self.latent_periods[channel],
                                                                                                    f'Ch{channel + 1}')

        return self.latent_period_figs
