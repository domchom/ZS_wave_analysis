...
class TotalSignalProcessor:
    ...
    def calc_peak_props(self):
        ...
        and polynomial order 2. The peak properties and latent period are calculated by finding peaks 
        in the smoothed signal, and then calculating the properties of each peak. Returns the average of 
        the signal in each box.

        Returns seven objects.. 
        ...
        self.latent_periods array of shape (channels, boxes) containing the number of frames in which the signal is below some threshold for the latent period
        self.ind_trough_props is a dictionary containing the threshold, left index, and right index for the latent period for each box in each channel
        '''
        ...
        self.latent_periods = np.zeros(shape=(self.num_channels, self.num_boxes))
        ...
        for channel in range(self.num_channels):
            for box_num in range(self.num_boxes):
                ...
                if len(peaks) > 0:
                    ...
                    # If more than two peaks detected, calculate the latent period
                    if len(peaks) > 1:
                        indv_latent_periods = []
                        indv_left_indexes = []
                        indv_right_indexes = []
                        ...
                        # store the latent period properties
                        self.latent_periods[channel, box_num] = mean_time_below_threshold
                        self.ind_trough_props[f'Ch {channel} Box {box_num}'] = {'heights': thresholds, 
                                                            'leftIndex': indv_left_indexes, 
                                                            'rightIndex': indv_right_indexes}
                    
                    # If only one peak detected
                    else: 
                        # store the latent period properties
                        self.latent_periods[channel, box_num] = np.nan
                        self.ind_trough_props[f'Ch {channel} Box {box_num}'] = {'heights':  np.nan, 
                                                            'leftIndex':  np.nan, 
                                                            'rightIndex':  np.nan}
        ...
        return self.peak_widths, self.peak_maxs, self.peak_mins, self.peak_amps, self.peak_rel_amps, self.ind_peak_props, self.latent_periods, self.ind_trough_props
    ...
    def plot_ind_peak_props(self):
        ...
        def return_figure(box_signal: np.ndarray, prop_dict: dict, trough_dict: dict, Ch_name: str):
            ...
            thresholds = trough_dict['heights']
            left_latent_Index = trough_dict['leftIndex']
            right_latent_Index = trough_dict['rightIndex']
            ...
            # plot the latent periods
            if not type(thresholds) == float:
                for i in range(len(thresholds)):
                    ax.hlines(thresholds[i], 
                            left_latent_Index[i], 
                            right_latent_Index[i], 
                            color='tab:brown', 
                            linestyle = '-')
            ...
            if not type(thresholds) == float:
                ax.hlines(thresholds[0], 
                            left_latent_Index[0], 
                            right_latent_Index[0], 
                            color='tab:brown', 
                            linestyle = '-',
                            label = 'latent period')
                ...
                for channel in range(self.num_channels):
                    for box in range(self.num_boxes):
                        ...
                        self.ind_peak_figs[f'Ch{channel + 1} Box{box + 1} Peak Props'] = return_figure(self.means[:,channel, box],
                                                                                                    ...
                                                                                                    self.ind_trough_props[f'Ch {channel} Box {box}'],
    ...
    def organize_measurements(self):
        ...
        if hasattr(self, 'acfs'):
            ...
            self.latent_period_with_stats = add_stats(self.latent_periods, 'Latent Period')
            for channel in range(self.num_channels):
                statified_measurements.append(self.periods_with_stats[channel])
                statified_measurements.append(self.latent_period_with_stats[channel])
        ...
        if hasattr(self, 'peak_widths_with_stats'):
            ...
            for channel in range(self.num_channels):
                ...
                for ind, stat in enumerate(stats_location):
                    ...
                    self.file_data_summary[f'Ch {channel + 1} {stat} Latent Period'] = self.latent_period_with_stats[channel][ind + 1]
